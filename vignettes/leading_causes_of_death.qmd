...
- Infant death analyses are special.
  ...
  - In addition to using a different set of cause of death functions, *infant death analyses do not use the standard death certificate & population data* available from `rads::get_data_death()` and `apde.data::population()`.
...
pop <- apde.data::population(years = 2017:2021, ages = 1:100, group_by = c('genders', 'ages'))
