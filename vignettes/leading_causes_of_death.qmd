...
- Infant death analyses are special.
  ...
  - In addition to using a different set of cause of death functions, *infant death analyses do not use standard death certificate & population data*.
    ...
    - The ***numerator*** comes from the infant death files. In Washington State, they are downloaded from [secureaccess.wa.gov/doh/chsdatafiles/](https://secureaccess.wa.gov/doh/chsdatafiles/).
  ...
  - Infant deaths for a given geography include any infant who dies and **either** was born to a mother who lived in that geography at the time of birth **or** was an infant who resided in that geography at the time of death.
...
The code below provides an example of how to conduct a leading causes of death analysis using RADS. For the sake of simplicity, we limit the stratification to gender (female & male) for the years 2019 to 2023.
...
infdeath <- infdeath[, .(year, sex, age, underlying_cod_code)]
...
Be sure that your call to `apde.data::population` provides the same years and demographic strata that you have in your death counts!
