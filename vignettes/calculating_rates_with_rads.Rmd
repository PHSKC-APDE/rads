...
Glancing at the output from the `death_injury_matrix_count` function, we see the that we have **aggregated** `deaths` by `age` and stratified by `intent`. This is all we really need. Let's tidy our data set and rename the `deaths` column:
...
Let's load some populatoin data using `rads.data::synthetic_pop`
...
Here we see that the King County **crude** and **adjusted** fall death rates relatively similar. We also see that the vast majority of fall related deaths are unintentional. 
...
This table of deaths by race shows that the number of deaths among White residents (`r format(sum(deaths[race == 'White']$count), big.mark = ',')`) is more than 18 times higher than that among the next highest racial group (Asian = `r sum(deaths[race == 'Asian']$count)`). Does that mean the death *rates* among White residents are are also exceptionally high? Let's get the corresponding population data and find out.
