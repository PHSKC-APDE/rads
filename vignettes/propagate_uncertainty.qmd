...
format:
  ...
  gfm:
    prefer-html: false
vignette: >
  %\VignetteIndexEntry{propagate_uncertainty()}
...
print(paste0("Traditional method for difference: ", 
             round(diff_traditional, 2),
            " 95% CI: (", round(ci_lower_traditional, 2), ", ", 
            round(ci_upper_traditional, 2), ")"))
```
...
Let's see how Monte Carlo simulation works by comparing it with the familiar case of estimates from normal distributions. These are the conditions under which traditional parametric methods work.
...
mortality_result <- propagate_uncertainty(
  ...
  contrast_fn = function(x, y) x - y,
  dist = "lognormal",                   # For rates with asymmetric CI
  draws = 10000,
  seed = 98104
)
```
...
Imagine we ran a logistic regression predicting the odds of knowing how to play saxophone across regions, using the Western Region as the reference group. We want to compare the East Region effect to the North Region effect by calculating the ratio of their odds ratios. To do so, we'll compare a manual method against using `propagate_uncertainty()`, showing that they produce nearly equivalent results.
...
sax_data <- data.table(
  comparison = "East Region OR vs North Region OR",
  east_or = 1.85,        # OR for East vs West Regions
  east_lower = 1.42,     
  east_upper = 2.41,
  north_or = 1.34,       # OR for North vs West Regions  
  north_lower = 1.08,    
  north_upper = 1.66
)
```
...
east_log_or <- log(sax_data$east_or)
north_log_or <- log(sax_data$north_or)

# Calculate approximate SEs from CIs on log scale
east_se_log <- (log(sax_data$east_upper) - log(sax_data$east_lower)) / 
                (2 * 1.96)

north_se_log <- (log(sax_data$north_upper) - log(sax_data$north_lower)) / 
                 (2 * 1.96)

# Everything is now on log scale, so use traditional methods
...
easy_way_result <- propagate_uncertainty(
  ...
  comp_mean_col = "east_or",        # East:West OR
  comp_lower_col = "east_lower",
  comp_upper_col = "east_upper",
  ref_mean_col = "north_or",        # North:West OR
  ref_lower_col = "north_lower", 
  ref_upper_col = "north_upper",
