---
title: "Uncertainty Propagation with propagate_uncertainty()"
format: gfm
prefer-html: false
self-contained: true
editor: visual
...
Let's see how Monte Carlo simulation works, first with a familiar normal case, then with a more complex scenario.

## Example 1: Normal Distributions (Traditional Methods Work)
...
plot_data <- data.table(
  ...
  distribution = factor(rep(c("Megalopolis LE", "Smallville LE", "Difference"), each = 10000),
...
## Example 2: Skewed Distributions (Traditional Methods Fail)

Define the age-adjusted mortality rates with asymmetric confidence intervals
```{r}
smallville_rate <- 650
smallville_lower <- 620
smallville_upper <- 695

megalopolis_rate <- 680  
megalopolis_lower <- 655
megalopolis_upper <- 720
```

Traditional calculation (**problematic for asymmetric CIs!**)
```{r}
traditional_diff <- megalopolis_rate - smallville_rate

smallville_se_approx <- (smallville_upper - smallville_lower) / (2 * 1.96)
megalopolis_se_approx <- (megalopolis_upper - megalopolis_lower) / (2 * 1.96)

traditional_se <- sqrt(smallville_se_approx^2 + megalopolis_se_approx^2)

traditional_lower <- traditional_diff - 1.96 * traditional_se
traditional_upper <- traditional_diff + 1.96 * traditional_se
```

Monte Carlo simulation using lognormal distributions
```{r}
set.seed(98104)
n_draws <- 10000

# Convert asymmetric CIs to lognormal parameters
smallville_sdlog <- (log(smallville_upper) - log(smallville_lower)) / (2 * 1.96)
megalopolis_sdlog <- (log(megalopolis_upper) - log(megalopolis_lower)) / (2 * 1.96)

# Generate Monte Carlo samples using lognormal (creates skewed distributions)
smallville_draws <- rlnorm(n_draws, log(smallville_rate), smallville_sdlog)
megalopolis_draws <- rlnorm(n_draws, log(megalopolis_rate), megalopolis_sdlog)
difference_draws <- megalopolis_draws - smallville_draws
```
...
plot_data2 <- data.table(
  value = c(megalopolis_draws, smallville_draws, difference_draws),
  distribution = factor(rep(c("Megalopolis Rate", "Smallville Rate", "Difference"), each = 10000),
                       levels = c("Megalopolis Rate", "Smallville Rate", "Difference"))
)
...
# Create labels for each panel
labels_data2 <- data.table(
  distribution = factor(c("Megalopolis Rate", "Smallville Rate", "Difference"),
                       levels = c("Megalopolis Rate", "Smallville Rate", "Difference")),
  label = c(
    paste0("Mean: ", round2(megalopolis_mean, 1), "\nCI: (", round2(megalopolis_ci_low, 1), ", ", round2(megalopolis_ci_high, 1), ")"),
    paste0("Mean: ", round2(smallville_mean, 1), "\nCI: (", round2(smallville_ci_low, 1), ", ", round2(smallville_ci_high, 1), ")"),
    paste0("Mean: ", round2(mc_diff, 1), "\nCI: (", round2(mc_lower, 1), ", ", round2(mc_upper, 1), ")")
  )
)

p2 <- ggplot(plot_data2, aes(x = value, fill = distribution)) +
  geom_histogram(alpha = 0.7, bins = 100, color = "white") +
  facet_wrap(~distribution, ncol = 3, scales = "free_x") +
  labs(title = "Monte Carlo Simulation: Skewed Case",
       subtitle = "Comparing Age-Adjusted Mortality Rates (per 100,000)",
       x = "Value", y = "Count") +
  theme_minimal() +
  theme(legend.position = "none",
        panel.grid = element_blank()) +
  geom_text(data = labels_data2, aes(label = label), 
            x = Inf, y = Inf, hjust = 1.1, vjust = 1.1, 
            size = 3, inherit.aes = FALSE)
...
#| echo: false
pretty_kable(data.table(` ` = c('Difference', 'Lower', 'Upper'), 
                        Traditional = round2(c(traditional_diff, traditional_lower, traditional_upper), 1), 
                        `Monte Carlo` = round2(c(mc_diff, mc_lower, mc_upper), 1)))
```

Notice how the underlying distributions are skewed (right-tailed), and the resulting difference distribution is also skewed. The traditional method gives a symmetric confidence interval that doesn't capture this asymmetry, while Monte Carlo naturally handles it.
...
Often we want to calculate ratios rather than differences:
