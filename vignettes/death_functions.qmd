...
Now, identify deaths due to COVID-19. Remember COVID-19 is not one of the official 113 causes of death, so let's arbitrarily get causeid #1 (Salmonella infections), knowing that it will also give us COVID-19.
...
- `cause_name`: a character vector specifying the multicause death definition from the reference table. For example, "Opioid". To see available options, use `death_multicause()`.
- `underlying_codes`: a character vector of ICD10 codes that must appear in the underlying cause of death. Only used if `cause_name` is NULL. For example, c("X40", "X41", "X42").
- `contributing_codes`: a character vector of ICD10 codes that must appear in the contributing causes of death. Only used if `cause_name` is NULL. For example, c("T400", "T401").
...
### example #8: Count the number of heat stress deaths for 2017-2021, stratified by year

Get the data

```{r}
ex8 <- get_data_death(year = 2017:2021, 
...
setorder(dt8, chi_year, cause.of.death)
```
...
#| message: false
#| warning: false
dt9 <- death_other_count(ph.data = ex9, 
                         cause = "cancer|heart", 
                         kingco = FALSE, 
                         ypll_age = 65)
```
...
### example #10: Get life expectancy at birth for King County residents born 2016-2020, stratified by sex
...
#| message: false
#| warning: false
ex10 <- get_data_death(year = 2016:2020, 
...
#| message: false
#| warning: false
dt10 = life_table_prep(ph.data = ex10, 
                      group_by = 'chi_sex')
```
...
#| message: false
#| warning: false
dt10 <- dt10[!is.na(chi_sex) & !is.na(ages)]
```
...
```{r}
#| echo: false
kable(dt10.lifetable[ages == '0-1',], format = "markdown")
```
