...
Data model receives a data.table object and returns a data.table of similar structure and content, but with uncorrelated data across columns. For example, a column of dates will become a column of dates of a similar range, and distribution, but randomly distributed across observations. Common use cases include:

+  demonstrating a statistic or visualization without exposing the audience to "real" data or results
+  sharing code, including code to generate a workable data set, to request help from others
+  creating data sets on the fly for unit testing or training

There are two kinds of output that data_modeler() returns, either the data table itself, or a table of strings that can be evaluated to create the data table.

In addition, data_modeler() can output a human readable version of the generation code that the user can copy and paste into their own scripts for sharing publicly.

### data privacy caveats

data_modeler makes some guesses as to if a column's content should be randomized (such as person names or blood pressures) or kept but randomly distributed (such as levels of response to a question or city names). They way it does this is based on how unique the observation, and not based on specific content. Because of this, you should consider if and how a variable should first be de identified or otherwise generalized, hashed, or removed

data_modeler creates an uncorrelated column of data to substitute the original by randomly reassigning values across that column. However, this does not ensure that meaningful information is not passed along. For instance, if the provided data set is of outcomes in a small population (e.g. indicator A on American Indian/Alaska Native population in a given zip code) the uncorrelated result would still show a similar rate of that outcome in that population. The analyst should consider if this is ok or not and take measures to correct if necessary by either changing the input data or modifying the synthetic data. A simple approach in the above example would be to add one or more population groups so that the outcome would be distributed (in an uncorrelated distribution) across both populations, removing the ability to estimate the rate for the smaller population.

### walkthrough

For the below examples, we use teh following demonstration data. But this should work with any two dimensional data.table object (or data frames that are coercable to a data.table). Our demo "real" data has one indicator variable that represents an outcome that is never in the white population, sometimes in the asian population, and always in other races. In an uncorrelated (and so non-informative) synthetic data set, we would expect this indicator to no longer be biased by race.
...
# and we will attach biased indicators.
#indicator1, all White population are "never", all "Asians, "sometimes", and everyone else, "always
...
DT.synthetic.data <- data_modeler(ph.data = DT.real.data, #our "real" data
                                ...
                                comments = TRUE, #weather or not to include comments in code (only matters for return_code == TRUE or print_code == TRUE)
                                return_code = FALSE, #return a DT of strings for creating the synthetic data instead of the data itself. leave FALSE to receive synthetic data
...
But the data shapes are not:

```{r}
#| echo: fenced
table(DT.real.data[,.(indicator1,chi_race_7)])
table(DT.synthetic.data[,.(indicator1,chi_race_7)])
```

You can review, share, and execute the code used to create the synthetic data in two ways. Manually by printing it to terminal and copying it into your script, or receiving and processing the text object, which is more useful or programmatic handling.
...
DT.synthetic.data <- data_modeler(ph.data = DT.real.data,
                                ...
                                comments = TRUE, 
                                return_code = FALSE,
                              print_code = TRUE) # change this to true
```
The below code is the same text used to generate the data.table we assigned to DT.synthetic.data

```{r code from terminal}
DT <- data.table( 
`id` = as.integer(1:100), # as ID variable integer (non factor) 
`chi_geo_kc` = sample(c(NA, "King County"), 100, replace = TRUE, prob = c(0.482, 0.518)), # as categorical character (non factor) 
`chi_race_7` = factor(sample(c("White", "Multiple", "Hispanic", "AIAN", "Black", "Asian", "Other", NA, "NHPI"), 100, replace = TRUE, prob = c(0.179, 0.016, 0.07, 0.115, 0.005, 0.384, 0.057, 0.152, 0.022)), levels = c("Asian", "AIAN", "Black", "Hispanic", "NHPI", "White", "Other", "Multiple"), ordered = FALSE), # as factor 
`chi_sex` = factor(sample(c("Female", "Male"), 100, replace = TRUE, prob = c(0.525, 0.475)), levels = c("Female", "Male"), ordered = FALSE), # as factor 
`chi_geo_region` = factor(sample(c("Seattle", "East", "South", "North"), 100, replace = TRUE, prob = c(0.284, 0.232, 0.243, 0.241)), levels = c("South", "North", "Seattle", "East"), ordered = FALSE), # as factor 
`indicator1` = sample(c("never", "always", "sometimes", NA), 100, replace = TRUE, prob = c(0.415, 0.384, 0.179, 0.022))) # as categorical character (non factor)
```

This allows you to review the logic, share the code with others, or make changes as you see fit. Note that this output includes comments that describe the nature of each variable. Assign "comments = FALSE" to exclude these from the output.

If you wish to use R to edit the text, you can receive the text as a DT object instead using:
...
DT.synthetic.data.instructions <- data_modeler(ph.data = DT.real.data,
                                number_of_observations =  100, 
                                comments = FALSE, 
                                return_code = TRUE, #setting this to "TRUE" will a DT of text strings instead of a DT of synthetic data
                              print_code = FALSE)
