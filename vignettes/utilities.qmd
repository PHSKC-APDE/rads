...
- [`bin_age()`](#bin_age): Bin single-year ages into the age categories used for age-standardization
...
`age_standardize()` will automatically bin ages to align with those in a reference population. However, *if* you ever find yourself needing to manually create age bins, `bin_ages()` will be your friend. `bin_age()` collapses single-year ages into the `agecat` bins used for age-standardization, either matching a standard reference population (see `list_ref_pop()` and `get_ref_pop()`) or a custom set of user-supplied age bins. It is the same binning logic that `age_standardize()` uses internally when `collapse = TRUE`. If neither `ref.popname` nor `cuts` is specified, `bin_age()` defaults to the same reference population that `age_standardize()` uses by default: "2000 U.S. Std Population (11 age groups)".

### Binning to a standard reference population

```{r}
#| echo: true
#| results: 'hide'
ages <- data.table(age = c(0, 2, 7, 22, 48, 63, 91))
ages[, agecat := bin_age(age, ref.popname = "2000 U.S. Std Population (11 age groups)")]
head(ages)
```

```{r}
#| echo: false
pretty_kable(head(ages))
```

Because the bins and `agecat` labels come directly from `get_ref_pop()`, the resulting `agecat` values are guaranteed to align with the standard population that will later be used in `age_standardize()`.

### Binning to custom age bins

If none of the standard reference populations fit your analysis, supply your own bin starting points via `cuts`. The largest value becomes the start of an open-ended top bin:

```{r}
#| echo: true
#| results: 'hide'
ages[, agecat_custom := bin_age(age, cuts = c(0, 18, 65))]
head(ages)
```
...
`ref.popname` and `cuts` cannot both be specified. Ages younger than the youngest bin (as well as `NA` ages) cannot be assigned to a bin and are returned as `NA` with a warning. This is a useful signal that your `cuts` (or age data) may have issues.
...
`data_modeler()` makes some guesses as to if a column's content should be randomized (such as person names or blood pressures) or kept but randomly distributed (such as levels of response to a question or city names). This is intended to generate random data or to retain original categories where appropriate. It bases these guesses on the data type (integer or string), and complexity (e.g. 75% being unique data), but not the meaning of the data (i.e. it doesn't know that a number is a "zip code" and doesn't know that a string is a "human name"). Variables that are identified as non-categorical will, in most cases, be replaced with a uniform distribution of noisy, but similarly shaped, data (e.g. floating point numbers of the same range and precision, but in a random uniform distribution, or a "lorem ipsum" string of similar lengths). Variables identified as categorical will, in most cases, be replaced with identical categories in a similar distribution to the original, but randomly assigned across observations (a data set of 95% males and 5% females would still generate a data set of roughly 95% males and 5% females).
...
- "categorical_threshold": defines how many unique categories can exist in the variable before data_modeler assumes the variable isn't categorical. Above this number, the value will not be considered categorical. This currently defaults to 61.
...
DT.real.data <- data.table(
   id = 1:1000,
   geo_county = sample(c("County1", "County2"), 100, replace = TRUE, prob = c(0.494, 0.506)),
   race = factor(sample(c("Asian", "AIAN", "Black", "Hispanic", "NHPI", "White", "Other", "Multiple", NA), 1000, replace = T, prob = c(.19,.01,.07,.11,.01,.35,.07,.14,.02)), levels = c("Asian", "AIAN", "Black", "Hispanic", "NHPI", "White", "Other", "Multiple", NA)),
   sex = as.factor(sample(c("Male","Female"), 1000, replace = T)),
   geo_region = factor(sample(c("North", "South", "East", "West"), 100, replace = TRUE, prob = c(0.241, 0.229, 0.258, 0.272)), levels = c("North", "South", "East", "West"), ordered = FALSE))
...
#indicator1, all White population are "never", all Asians population, "sometimes", and everyone else, "always"
   DT.real.data[, indicator1 := ifelse(race == "White", "never",
                         ifelse(race == "Asian", "sometimes",
                                "always"))]
```
...
```{r}
#| echo: fenced
identical(str(DT.real.data), str(DT.synthetic.data))
```

But we can see that the original bias of Whites in the never group, Asians in the sometimes group, and all others in the always group now no longer holds.

```{r}
#| echo: true
prop.table(table(DT.real.data[,.(indicator1,race)]))
prop.table(table(DT.synthetic.data[,.(indicator1,race)]))
```
...
DT.synthetic.data <- data_modeler(ph.data = DT.real.data,
                                  ...
                                  comments = TRUE, 
                                  return_code = FALSE,
                                  print_code = TRUE) # TRUE allows you to copy code from your terminal output
```
...
DT.synthetic.data.instructions <- data_modeler(ph.data = DT.real.data,
                                number_of_observations =  100, 
                                comments = FALSE, 
                                return_code = TRUE, #TRUE will cause data_modeler() to return a list object of the text instead of a DT of synthetic data
                                print_code = FALSE)
...
1.  **Flexible degrees of freedom calculation**: The function offers four methods for calculating degrees of freedom:
    ...
    - "**liberal**": Assumes infinite degrees of freedom (df = Inf); higher Type I Error (false +) and lower Type II Error (false -)
