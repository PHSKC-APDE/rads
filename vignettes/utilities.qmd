...
For the examples below, we create a "real" data set and show how to use `data_modeler()` and how the synthetic and "real" data differ. following demonstration data. Our demo "real" data has one indicator variable that represents an outcome that is never in the White population, sometimes in the Asian population, and always in other Races. In an uncorrelated (and so non-informative) synthetic data set, we would expect this indicator to no longer be biased by race.
