...
First, create a simple tabulation of the population of Seattle after sub-setting PUMS to adults (age 18+).
...
-   `mean`: The proportion of the total population (i.e., adults) that lives in Seattle
-   `total`: The survey weighted population of Seattle and seattle residents among adults
-   `numerator`: The number of rows with people living in Seattle, compare to `nrow(mysurvey[seattle == 'Seattle'])`
-   `denominator`: The number of rows in the dataset where the 'what' variable can be assessed because it is not missing, compare to `nrow(mysurvey[!is.na(seattle)])`
