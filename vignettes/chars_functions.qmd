...
format:
  ...
  gfm:
    prefer-html: false
vignette: >
  %\VignetteIndexEntry{CHARS Functions}
...
```{r}
#| echo: false
  pretty_kable(chars_icd_ccs()[1:3])
```
...
```{r}
#| echo: false
  pretty_kable(chars_icd_ccs(ref_type = 'broad'))
```
...
```{r}
#| echo: false
pretty_kable(chars_icd_ccs()[grep('I110', icdcm_code)])
```
...
```{r}
#| echo: false
  pretty_kable(chars_injury_matrix()[1:10])
```
