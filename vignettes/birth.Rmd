...
## Installation
You will need to install [R](https://cran.r-project.org/bin/windows/base/), [Rstudio](https://rstudio.com/products/rstudio/download/#download), and the [`devtools`](https://cran.r-project.org/web/packages/devtools/index.html), [`apdeRecodes`](https://github.com/PHSKC-APDE/apdeRecodes) and [`rads`](https://github.com/PHSKC-APDE/rads) packages. If needed, 
...
3) Install `apdeRecodes` by typing `devtools::install_github("PHSKC-APDE/apdeRecodes")` in the R console. 
    ...
    * After following the instructions above, please try typing the following: `devtools::install_github("PHSKC-APDE/apdeRecodes", auth_token = Sys.getenv("GITHUB_PAT"))`
4) Install `rads` by typing `devtools::install_github("PHSKC-APDE/rads")` in the R conole. 
    * Again, if you see a 404 error, try again using authentication, `devtools::install_github("PHSKC-APDE/rads", auth_token = Sys.getenv("GITHUB_PAT"))`
5) Exit R studio and starting it again. You should now be ready to walk through this vignette. 
