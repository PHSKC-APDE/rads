...
## Installation
You will need to install [R](https://cran.r-project.org/bin/windows/base/), [Rstudio](https://rstudio.com/products/rstudio/download/#download), and the [`rads`](https://github.com/PHSKC-APDE/rads) package. If needed, install R and Rstudio the way you would install any other software. Afterward, install the devtools package in R by typing `install.packages("devtools")` in the R console. After successfully installing `devtools`, install `rads` by typing `devtools::install_github("PHSKC-APDE/rads")`. After doing this, I recommend shutting down R studio and starting it again. You should now be ready to walk through this vignette. 
...
args(calc)
```
This is because calc is a front end for two engines: [`calc.data.table`](https://github.com/PHSKC-APDE/rads/blob/master/R/calc.data.table.R) for record level data & [`calc.tbl_svy`](https://github.com/PHSKC-APDE/rads/blob/master/R/calc.tbl_svy.R) for survey data. The good news is that you can ignore this and use the following standard set of arguments:
...
**calc 5 year**
```{r}
birth <- calc(birth, what = c("preterm_lbw"), metrics = c("mean", "lower", "upper", "se", "rse", "caution", "numerator",
                                                          "denominator", "total", "missing", "missing.prop", "unique.years") )
head(birth)
```
...
**get data**
```{r}
birth <- get_data_birth(year = c(2014:2018), cols = c("chi_year", "preterm", "chi_race_eth8", "mother_birthplace_foreign"), kingco = T)

```
**calc 5 year**
```{r}
birth_5y <- calc(birth, what = "preterm", by = c("chi_race_eth8", "mother_birthplace_foreign"), metrics = c("mean", "lower", "upper", "se", "rse", "caution", "numerator"))
head(birth_5y)
```
...
**calc 1 year**
```{r}
birth_1y <- calc(birth, what = "preterm", by = c("chi_race_eth8", "mother_birthplace_foreign", "chi_year"), metrics = c("mean", "lower", "upper", "se", "rse", "caution", "numerator"))
head(birth_1y)
```
...
### Premature by race in a specific area
**get data** 
```{r}
birth <- get_data_birth(year = c(2014:2018), cols = c("chi_year", "preterm", "chi_race_eth8", "chi_geo_big_cities", "chi_geo_hra_short", "chi_geo_zip5"))
```

**calc by each "big city"**
```{r}
birth_big <-  calc(birth, what = "preterm", by = c("chi_race_eth8", "chi_geo_big_cities"), metrics = c("mean", "lower", "upper", "se", "rse"))
head(birth_big)
```

**calc within a limited set of zip codes**
```{r}
birth_zip <- calc(birth, what = "preterm", chi_geo_zip5 %in% c(98001:98010), by = c("chi_race_eth8"), metrics = c("mean", "lower", "upper", "se", "rse"))
head(birth_zip)
```
**calc by HRAs like "Auburn"**
```{r}
# birth_hra <-  calc(birth, what = "preterm", chi_geo_hra_short %like% c("Auburn"), by = c("chi_race_eth8", "chi_geo_hra_short"), metrics = c("mean", "lower", "upper", "se", "rse"))
# head(birth_hra)      
```
...
**get data**
```{r}
birth <- get_data_birth(year = c(2014:2018), cols = c("chi_year", "chi_age", "chi_geo_hra_long", "chi_race_eth8", "dlp_medicaid", "prior_live_births_living", "prior_live_births_deceased"))
```
**calc Medicaid births, first time moms, under 23, by HRA and by race**
```{r}
birth_custom1 <- calc(birth, what = "dlp_medicaid", (prior_live_births_living + prior_live_births_deceased == 0) & chi_age < 23, by = c("chi_geo_hra_long", "chi_race_eth8"))
head(birth_custom1)
```
...
**recode data**
```{r}
birth[, Duamish := 0][chi_geo_hra_long %in% c("Beacon Hill/Georgetown/South Park", "Delridge"), Duamish := 1]
```
**calc % of all births that were in Duamish, by year**
```{r}
birth_custom_2 <- calc(birth, what = "Duamish", metrics = c("mean", "lower", "upper", "se", "rse"), by = c("chi_year"))
head(birth_custom_2)
```
