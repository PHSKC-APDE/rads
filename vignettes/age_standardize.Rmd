...
The data set already contains a `stdpop` column, so we can set `ref.popname = "none"`. Also note that we need to specify `collapse = F` because the function expects data to be pre-aggregated when I have the column `stdpop`.
