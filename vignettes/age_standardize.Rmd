...
5)  `my.pop`: a character vector, identifies the column name for the population number corresponding to the aggregated count. The default is "pop"
...
8)  `by`: a character vector, specifies the variable(s) by which to stratify the rate results (if any). The default is NULL (i.e., no stratification)

9)  `diagnostic_report`: If `by` is used and there are groups with missing ages, setting `diagnostic_report = TRUE` returns a diagnostic table instead of normal results. Use this option if a warning about missing age groups appears when running the function normally.
...
**Run age_standardize with the default reference population**
...
You're staffing data requests today and receive the following message: "*Hi! For a class, I'd like to get the 2019 15 to 18 birth rate for King County and WA State as a whole. Since the age distribution in the county may differ from the rest of the state, it would be appreciated if you could provide crude and age-standardized rates. If you could standardize to 'World (WHO 2000-2025) Std Million (single ages to 84)', that would be great! Thank you, S. Capstone*"

You quickly pull up [CHAT](https://www.doh.wa.gov/ForPublicHealthandHealthcareProviders/PublicHealthSystemResourcesandServices/CommunityHealthAssessmentandImprovement/CHAT) only to find that this specific indicator doesn't exist. You remember that [CHI](https://kingcounty.gov/chi/) has an adolescent birth rate indicator, but are downcast when you find that it applies to those 15 to 19. In desperation, you resign yourself to using `rads`.

**Get birth counts for 15 to 18 year olds in 2019**

```{r, warning=FALSE, message=FALSE, eval=TRUE, results='hide'}
  kcbirth <- rads.data::synthetic_birth[year == 2019 & geo_kc == 'King County', .(age, year)] 
  wabirth <- rads.data::synthetic_birth[year == 2019, .(age, year)]
  
  births <- rbind(kcbirth[, geo := "King County"], wabirth[, geo := "Washington State"])
  births <- births[age %in% 15:18]
  
  # collapse / aggregate
  births <- births[, .(births = .N), by = c("age", "geo")] 
  setorder(births, geo, age)
  head(births)
```
...
```{r, warning=FALSE, message=FALSE, eval=TRUE, results='hide'}
  kcpop <- rads.data::synthetic_population[gender == 'Female' &
                                             geo == 'King County' & 
                                             age %in% 15:18 & 
                                             year == 2019]  

  wapop <- rads.data::synthetic_population[gender == 'Female' &
  ...
  pop <- pop[, .(pop = sum(pop)), .(age, geo)] # aggregate because it has rows for each race
...
According to your analysis, the crude King County teen birth rate is lower than (`r rads::round2(ex5.1[geo=="King County"]$crude.rate, 1)` vs `r rads::round2(ex5.1[geo=="Washington State"]$crude.rate, 1)`) the WA State teen birth rate. This relationship remains largely unchanged after age-standardization (`r rads::round2(ex5.1[geo=="King County"]$adj.rate, 1)` vs `r rads::round2(ex5.1[geo=="Washington State"]$adj.rate, 1)`).

------------------------------------------------------------------------

### (OPTIONAL) Calculating the standard error (SE) and relative standard error (RSE)

The method used by rads to calculate the confidence intervals is that recommended by WA DOH, i.e., the [Fay-Feuer method](https://wonder.cdc.gov/controller/pdf/FayFeuerConfidenceIntervals.pdf). Unfortunately, this method does not have a corresponding SE. We consulted with a WA DOH biostatistician who said, **"*SEâ€™s are not particularly useful for approximating the sampling distribution on the scale of adjusted rates*."** We therefore calculate **approximations** of the SE and RSE, as suggested by various state health departments. This means that the SE will be internally inconsistent with the confidence intervals ... *c'est la vie!*
