...
#' @param categorical_threshold integer >= 1. Threshold for the number of unique values in a variable at or below which data_modeler() will treat the value as categorical, default 61.
#' @param proportional_threshold floating point between 0 and 1 inclusive. Threshold for the proportion of observations that must be unique, below which data modeler() will treat the value as categorical, default 0.75
#'
...
#' C) creating data sets programmatically for (unit) testing or training
#'
...
#' 1) "categorical_threshold": defines how many unique categories can exist in the variable before data_modeler assumes the variable isn't categorical. Above this number, the value will not be considered categorical. This currently defaults to 61.
...
#'     sex = as.factor(sample(c("Male","Female"), observations, replace = TRUE)),
#'     region = factor(sample(c("North", "South", "East", "West"), observations,
#'       replace = TRUE), levels = c("North", "South", "East", "West")),
  ...
  variable_modeler <- function(oneVariable, number_of_observations, varName = NA, comments = TRUE, categorical_threshold, proportional_threshold) {
    ...
    if(inherits(oneVariable, "character")){
      ...
      if(is.na(instructions) & (length(unique(oneVariable)) <= categorical_threshold | length(unique(oneVariable)) < (length(oneVariable) * proportional_threshold))){
        #if factor list is too long, generalize
        if(nchar(paste0( oneVariable, collapse = "")) > 98000) { #around 98000 will cause parse to crash. 30000 characters leaves space for probabilities and other instructions
          ...
          if(comments){
            instructions <- paste0(instructions, ' # as categorical character, generified (non factor)')
          }
        } else {
        # if factor list is not too long...
