...
test_that('Where elimiates everything', {
  ...
  expect_equal(r1, data.table(variable = 'api00', mean = NaN, level = NA, mean_se = NA_real_, mean_lower = NA_real_, mean_upper = NA_real_ ))

  #factor what, no time var
  r2 = calc(sur, what = 'awardsYes', where = yyy == -1, metrics = c('mean'), proportion = T, ci = .95)
  expect_equal(r2, data.table(variable = 'awardsYes', mean = NaN, level = NA, mean_se = NA_real_, mean_lower = NA_real_, mean_upper = NA_real_ ))
  ...
  expect_equal(r3[, .(variable, mean, level, mean_se, mean_lower, mean_upper)], #yyy is integer(0)
               data.table(variable = 'awardsYes', mean = NaN, level = NA, mean_se = NA_real_, mean_lower = NA_real_, mean_upper = NA_real_ ))
