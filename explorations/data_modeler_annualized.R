#' PROTOTYPE CODE: Generate synthetic data for use in public sharing or testing but with years
#'
#' @description
...
#' @param years integer with number of years to use
...
data_modeler_annualized <- function(ph.data, number_of_observations = 100, years = as.numeric(format(Sys.Date(), "%Y"))-1, comments = TRUE, return_code = FALSE, print_code = TRUE, seed = 98104) {
  ...
  if(!inherits(years, "numeric")){
    stop("year is not numeric. Year must be a number or a vector of numbers (e.g. 2025, 2022:2025, or c(2022. 2024, 2025))")
  }
  ...
  if(comments) {
    ...
    codeListParsed <- c(list("for(year in years) {"), list("set.seed(seed*year)"),list("DTIteration <- data.table("), list("modeled_year = year,"),gsub(" #", ", #", codeList[1:(length(codeList)-1)]), gsub(" #",") #",codeList[length(codeList)]), list("if(exists('DT')) {
    DT <- rbind(DT, DTIteration)
    } else {
     DT <- DTIteration
    } }"))
